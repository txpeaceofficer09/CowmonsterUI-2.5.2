...
--f:SetBackdrop( { bgFile = [[Interface\AddOns\CowmonsterUI_InfoBar\textures\bar_serenity]], edgeFile = nil, tile = true, tileSize = 32, edgeSize = 0, insets = { left = 0, right = 0, top = 0, bottom = 0 } } )
f.background = f:CreateTexture(nil, "BACKGROUND")
f.background:SetAllPoints(f)
f.background:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background")
...
local function GetNumGuildMembersOnline()
	...
	for i=1,total,1 do
		local _, _, _, _, _, _, online = GetGuildRosterInfo(i)
		if online == "true" then
			online = online + 1
		end
	end
